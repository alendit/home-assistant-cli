...
@pass_context
def listcmd(ctx: Configuration, devicefilter: str) -> None:
    ...
    if devicefilter == ".*":
        ...
        devicefilterre = re.compile(devicefilter)
...
def assign(
    ...
    if match:
        if match == ".*":
            ...
            devicefilterre = re.compile(match)
    ...
    for id_or_name in names:
        found_device: Optional[Dict[str, Any]] = next(
            (x for x in devices if x["id"] == id_or_name), None
        )
        if not found_device:
            found_device = next((x for x in devices if x["name"] == id_or_name), None)
        if not found_device:
...
def rename(
    ...
    if not device:
        device = next((x for x in devices if x["name"] == device_id_or_name), None)
    if not device:
